...
class SGC(Cell):
    """SGC class representing a spiral ganglion cell."""
    ...
    def create(cls, model="I", species="mouse", **kwds):
        ...
        ----------
        model : str, optional
            Model specification , by default "I"
        species : str, optional
            species for model, by default "mouse"
    ...
    def make_terminal(self, post_cell, term_type, **kwds):
        ...
        if term_type == "simple":
            ...
            n_rsites = data.get(
                "sgc_synapse", species="mouse", post_type=post_cell.celltype, field="n_rsites"
            )
            opts = {
                "nzones": n_rsites,
                "delay": 0,
            ...
            raise ValueError(f"Unsupported terminal type {term_type}")
...
class SGC_TypeI(SGC):
    ...
    def __init__(
        ...
        morphology : string (default: None)
            a file name to read the cell morphology from. If a valid file is found,
            a cell is constructed as a cable model from the hoc file.
        ...
        nach : string (default: 'na')
            nach selects the type of sodium channel that will be used in the model.
            A channel mechanim by that name must exist.
            The default is jsrna (Rothman et al., 1993)

        ttx : Boolean (default: False)
            If ttx is True, then the sodium channel conductance is set
            to 0 everywhere in the cell.
            Currently, this is not implemented.

        species: string (default 'guineapig')
            species defines the channel density that will be inserted for
        different models. Note that
            if a decorator function is specified, this argument is ignored.

        modelType: string (default: None)
            modelType specifies the type of the model that will be used. 
            SGC model know about "a" (apical)
        ...
        sr : string (default: None)
            The spontaneous rate group to which this fiber belongs. 
            "LS", "MS", and "HS" are known values.

        debug: boolean (default: False)
            debug is a boolean flag. When set, there will be multiple 
            printouts of progress and parameters.
        ...
        _ = self.do_morphology(morphology)
    ...
    def species_scaling(self, silent=True):
        ...
        if self.status["modelType"] == "sgc-a":
            soma().ihsgcApical.gbar = self.g_convert(
                self.pars.soma_ihap_gbar, self.pars.units, self.somaarea
            )
            soma().ihsgcApical.eh = self.pars.soma_ihap_eh
        elif self.status["modelType"] == "sgc-bm":
            soma().ihsgcBasalMiddle.gbar = self.g_convert(
                self.pars.soma_ihbm_gbar, self.pars.units, self.somaarea
            )
            soma().ihsgcBasalMiddle.eh = self.pars.soma_ihbm_eh
        else:
            raise ValueError(
                "Ihsgc modelType %s not recognized for species %s"
                % (self.status["modelType"], self.status["species"])
            )
