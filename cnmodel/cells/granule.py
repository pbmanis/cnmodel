    ...
    def make_psd(self, terminal, psd_type, **kwds):
        ...
        if psd_type == "simple":
            if terminal.cell.celltype in ["sgc", "dstellate", "tuberculoventral", "cartwheel"]:
                weight = data.get(
                    "%s_synapse" % terminal.cell.celltype,
                ...
                raise TypeError(
                    "Cannot make simple PSD for %s => %s" % (terminal.cell.celltype, self.celltype)
                )

        else:
            raise ValueError(
                "Unsupported psd type %s for cartwheel cell (inputs not implemented yet)" % psd_type
            )

    def make_terminal(self, post_cell, term_type, **kwds):
        if term_type == "simple":
            return synapses.SimpleTerminal(self.pre_sec, post_cell, **kwds)
        elif term_type == "multisite":
            if post_cell.celltype in ["tuberculoventral", "pyramidal"]:
                nzones = data.get(
                    "cartwheel_synapse",
                    species=self.species,
                    post_type=post_cell.celltype,
                    field="n_rsites",
                )
                delay = data.get(
                    "cartwheel_synapse",
...
class GranuleDefault(Granule):
    ...
    def __init__(
        ...
        """
        Create a granule cell model, based on a the cerebellar granule cell model from
        Diwakar et al., J. Physiology and/or Masoli et al. Communications Biology 2020.
        ...
        temp = 34
        self.c_m = 2.0 # From Masoli et al. 2020, and others from that lab
        print("modelname, modeltype: ", modelName, modelType)
        if species == "mouse":
            if modelType is None:
                modelType = "GRC"
            print("modelType", modelType)
            if modelName is None:  # references the column in the base table
                modelName = "GRC"
            if modelName == 'GRC':
                 basetable = 'GRC_channels'
            elif modelName  in 'GRC_channels_compartments':
                 basetable = 'GRC_channels'

            else:
                raise ValueError(
                    f"ModelName {self.status['modelName']:s} not recognized for {self.celltype:s} cells"
                )

        else:
            raise ValueError(f"Species {species:s} not recognized for {self.celltype:s} cells")
        print("modelType: ", modelType, " modelName: ", modelName, "  basetable: ",  basetable)
        self.debug = debug
        self._valid_temperatures = (temp,)
        self.status = {
            "species": species,
            "cellClass": self.celltype,
            "modelType": modelType,
            "modelName": modelName,
            self.somaname: True,
            "axon": False,
        ...
        print("Granule cell morphology built from ", morphology)

        self.pars = self.get_cellpars(basetable, species=species, modelType=modelType)
        ...
        if decorator is None:  # basic model, only on the soma
            ...
            for mech in self.mechanisms:
                print("Inserting mech: ", mech)
                self.soma.insert(mech)
            # self.soma.insert('cadiff')
            # self.soma.insert('GRCCALC')
            self.species_scaling()  # set the default cell parameters
    ...
    def get_cellpars(self, basetable, species="mouse", modelType="GRC"):
        pars = self.get_initial_pars(basetable, species, modelType)
        print("model name: ", self.status["modelName"], " basetable: ",  basetable)
        if self.status['modelName'] == 'GRC':
            for g in ["GRCNA_gbar",
                "GRCKV_gbar",
                "GRCKM_gbar",
                "GRCKA_gbar",
                ...
                pars.additem(g,  data.get( basetable, species=species, model_type=modelType,
                    field=g))
        elif self.status['modelName'] == 'GRC_channels_compartments':
            for g in ["GRCNA_gbar",
                    "GRCKV_gbar",
                    "GRCKM_gbar",
                    "GRCKA_gbar",
                    ...
                    "GRCLKG_gbar",
                    "GRCLKGG_gbar"]:
                print("g: ", g, " basetable: ",  basetable, "modeltype: ", modelType, "modelName: ", self.status['modelName'])
                try:
                    pars.additem(g,  data.get(self.status["modelName"], species=species, model_type=modelType,
                    parameter=g, compartment="soma"))
                except ValueError:
                    raise ValueError(f"Parameter {g:s} not found in { self.status['modelName']:s} for {species:s} {modelType:s}")
        else:
    ...
    def species_scaling(self, silent=True):
        ...
        self.vrange = [-90.0, -35.0]  # set a default vrange for searching for rmp

        self.set_soma_size_from_Cm(self.pars.cap)
        # print("self.pars: ", self.pars.todict())

        self.soma().GRCNA.gbar = self.g_convert(
            self.pars.GRCNA_gbar, self.pars.units, self.somaarea
        )
        self.soma().GRCKV.gbar = self.g_convert(
            self.pars.GRCKV_gbar, self.pars.units, self.somaarea
        )
        self.soma().GRCKA.gbar = self.g_convert(
            self.pars.GRCKA_gbar, self.pars.units, self.somaarea
        )
        self.soma().GRCKM.gbar = self.g_convert(
            self.pars.GRCKM_gbar, self.pars.units, self.somaarea
        )
        self.soma().GRCKIR.gbar = self.g_convert(
            self.pars.GRCKIR_gbar, self.pars.units, self.somaarea
        )
        self.soma().GRCKCA.gbar = self.g_convert(
            self.pars.GRCKCA_gbar, self.pars.units, self.somaarea
        )
        self.soma().GRCCA.gbar = self.g_convert(
            self.pars.GRCCA_gbar, self.pars.units, self.somaarea
        )
        self.soma().GRCLKG.gbar = self.g_convert(
            self.pars.GRCLKG_gbar, self.pars.units, self.somaarea
        )
        self.soma().GRCLKGG.gbar = self.g_convert(
            self.pars.GRCLKGG_gbar, self.pars.units, self.somaarea
        )
        # print("leak and gleak gbar: ", self.soma().GRCLKG.gbar, self.soma().GRCLKGG.gbar)
        # print("params: ", self.pars.todict())

        self.soma().ena = self.pars.e_na  # 50
        self.soma().ek = self.pars.e_k  # -80
        self.soma().eca = self.pars.e_ca  # 50
        self.soma().GRCLKG.e_l = self.pars.e_l
        self.soma().GRCLKGG.e_gaba = self.pars.e_gaba
    ...
    def i_currents(self, V):
        ...
        self.ix = {}
        # print(dir(self.soma))
        # print(self.soma.psection()['density_mechs'].keys())
        sec_mechs = list(self.soma.psection()["density_mechs"].keys())
        if "GRCNA" in sec_mechs:
            self.ix["GRCNA"] = self.soma().GRCNA.gna * (V - self.soma().ena)
        if "GRCKV" in sec_mechs:
            self.ix["GRCKV"] = self.soma().GRCKV.gk * (V - self.soma().ek)
        if "GRCKA" in sec_mechs:
            self.ix["GRCKA"] = self.soma().GRCKA.gk * (V - self.soma().ek)
        if "GRCKM" in sec_mechs:
            self.ix["GRCKM"] = self.soma().GRCKM.gk * (V - self.soma().ek)
        if "GRCKCA" in sec_mechs:
            self.ix["GRCKCA"] = self.soma().GRCKCA.gk * (V - self.soma().ek)
        if "GRCKIR" in sec_mechs:
            self.ix["GRCKIR"] = self.soma().GRCKIR.gk * (V - self.soma().ek)
        if "GRCCA" in sec_mechs:
            self.ix["GRCCA"] = self.soma().GRCCA.gca * (V - self.soma().eca)
        if "GRCLKG" in sec_mechs:  # resistive leak
            self.ix["GRCLKG"] = self.soma().GRCLKG.gbar * (V - self.soma().GRCLKG.e_l)
        if "GRCLKGG" in sec_mechs:  # resting GABA conductance
            self.ix["GRCLKGG"] = self.soma().GRCLKGG.gbar * (V - self.soma().GRCLKGG.e_gaba)
        # if 'GRCCALC' in sec_mechs:
        #      self.ix['GRCCALC'] = self.soma().GRCCALC.gca*(V - self.soma().GRCCALC.eca)
        # leak
        # if 'lkpkj' in sec_mechs:
