    ...
    def make_psd(self, terminal, psd_type, **kwds):
        ...
        if psd_type == "simple":
            ...
            if terminal.cell.celltype == "sgc":
                ...
                self.Pr = data.get(
                    "sgc_synapse", species=self.species, post_type=self.celltype, field="Pr"
                )
                ...
                raise TypeError(
                    "Cannot make PSD for %s => %s" % (terminal.cell.celltype, self.celltype)
                )
        else:
...
class OctopusRothman(Octopus, Cell):
    ...
    def __init__(
        ...
        if species == "guineapig":
            ...
            if modelName == "XM13":
                ...
                raise ValueError(
                    f"ModelName {self.status['modelName']:s} not recognized for mouse T-stellate cells"
                )
        else:
    ...
    def get_cellpars(self, dataset, species="guineapig", modelType="I-c"):
        ...
        if self.status["modelName"] == "RM03":
            for g in [
                ...
                pars.additem(g, data.get(dataset, species=species, model_type=modelType, field=g))
        ...
        #             field=g))
            ...
            raise ValueError(
                f"get_cellpars: Model name {self.status['modelName']} is not yet implemented for cell type {self.celltype.title():s}"
            )
    ...
    def species_scaling(self, silent=True):
        ...
        if self.status["species"] == "guineapig" and self.status["modelType"] == "II-o":
            ...
            sf = 1.0
            if self.status["temperature"] == 38.0:  # adjust for 2003 model conductance levels at 38
                sf = 3.03  # Q10 of 2, 22->38C. (p3106, R&M2003c)
                # note that kinetics are scaled in the mod file.
            self.adjust_na_chans(soma, sf=sf)
            soma().kht.gbar = sf * self.g_convert(
                150.0, self.pars.units, self.somaarea
            )  # 6.1 mmho/cm2
            soma().klt.gbar = sf * self.g_convert(
                1000.0, self.pars.units, self.somaarea
            )  #  40.7 mmho/cm2  3195?
            soma().ihvcn.gbar = sf * self.g_convert(
                30.0, self.pars.units, self.somaarea
            ...
            soma().klt.gbar = self.g_convert(
                3196.0, self.pars.units, self.somaarea
            )  #  40.7 mmho/cm2
            soma().hcnobo.gbar = self.g_convert(
                40.0, self.pars.units, self.somaarea
...
class OctopusSpencer(Octopus, Cell):
    ...
    def __init__(
        ...
        if modelType == None:
            modelType = "Spencer"
        # added 'modelName' so decorator.py line 35 doesn't KeyError
        self.status = {
            self.somaname: True,
            "axon": False,
            "dendrites": False,
    ...
    #  added modelName=None so decorator call (modelName=..., modelType=...) doesn't TypeError
    def channel_manager(self, modelName=None, modelType="Spencer"):
        """
        ...
        if modelType == "Spencer":
            ...
            self._valid_temperatures = (34.0,)  # 34 for consistency with other mouse models, but
            ...
            # keys changed from bare mech names ('klt') to
            # 'mech_gbar' format so _biophys split('_')[1] doesn't IndexError
            self.channelMap = {
                self.somaname: {
                    "jsrna_gbar": self.gBar.nabar,
                    "klt_gbar": self.gBar.kltbar_soma,
                    "kht_gbar": self.gBar.khtbar_soma,
                    "hcnobo_gbar": self.gBar.ihbar_soma,
                    "leak_gbar": self.gBar.leakbar,
                },
                "hillock": {
                    "jsrna_gbar": 0.0,
                    "klt_gbar": self.gBar.kltbar_hillock,
                    "kht_gbar": self.gBar.khtbar_hillock,
                    "hcnobo_gbar": self.gBar.ihbar_hillock,
                    "leak_gbar": self.gBar.leakbar,
                },
                # axon initial segment:
                "unmyelinatedaxon": {
                    "jsrna_gbar": self.gBar.nabar_ais,
                    "klt_gbar": self.gBar.kltbar_ais,
                    "kht_gbar": self.gBar.khtbar_ais,
                    "hcnobo_gbar": self.gBar.ihbar_ais,
                    "leak_gbar": self.gBar.leakbar,
                },
                "primarydendrite": {
                    "jsrna_gbar": 0.0,
                    "klt_gbar": self.gBar.kltbar_dend,
                    "kht_gbar": self.gBar.khtbar_dend,
                    "hcnobo_gbar": self.gBar.ihbar_dend,
                    "leak_gbar": self.gBar.leakbar,
                },
            }
