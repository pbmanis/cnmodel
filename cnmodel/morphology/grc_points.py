...
n_hillock_sections = 5
n_axon_sections = 30
n_axon_segments = 3
n_dendrites = 4
n_dendrite_sections = 4
n_soma_sections = 1

total_sections = (n_dendrite_sections*n_dendrites + n_hillock_sections + n_axon_sections + n_soma_sections)
...
x = 0
y = 0
z = 0 # starting z, but keep track of it
secpos = 0
...
def map_dend_dirs(dnum, secnum, xd0, yd0, secpos):
    ...
    if dend_dirs[dnum] == 'N':
        ...
        xd = xd0 - seclen
    print(f"dendrite:  {dnum:2d}, {secnum:2d}, xd0,yd0: {xd0:5.1f}, {yd0:5.1f}, {xd:5.1f}, {yd:5.1f}, {secpos:3d}")
    return xd, yd
...
for dend_num in range(n_dendrites):  # for each of 4 dendrites
    ...
    for section_number in range(2):  # for each section in the primary dendrite
        lprint(f"access sections[{secpos}]")
        lprint(f"primarydendrite.append()")
        xd, yd = map_dend_dirs(dend_num, section_number, xd0, yd0, secpos)
        lprint(f"sections[{secpos}] {{")
    ...
    # add claw
    lprint(f"access sections[{secpos}]")
    lprint(f"dendriticclaw.append()")
    lprint(f"sections[{secpos}] {{")
    xd, yd = map_dend_dirs(dend_num, 3, xd0, yd0, secpos)
    lprint(f"    pt3dadd({xd0:.2f}, {yd0:.2f}, {z:.2f}, {diam:.2f})")
    lprint(f"    pt3dadd({xd:.2f}, {yd:.2f}, {z:.2f}, {diam:.2f})")
    lprint("}")
    lprint(f"connect sections[{secpos:d}](0), sections[{secpos-1:d}](1)\n")
