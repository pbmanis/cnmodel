...
def testexp():
    ...
    """
    pars = {"tau1": 0.1, "tauratio": 2.0, "weight": 0.1, "erev": -70.0, "v": -65.0, "delay": 1}
    F = Exp2SynFitting(
        initpars={
            "tau1": 0.2,
            "tauratio": 2.0,
    ...
    target = F.exp2syn(
        t, pars["tau1"], pars["tauratio"], pars["weight"], pars["erev"], pars["v"], pars["delay"]
    )
...
def compute_psc(synapsetype="multisite", celltypes:Union[list, None]=None):
    """
...
def fit_one(st, stk):
    ...
    if stk[0] in ["sgc", "tstellate", "granule"]:
        erev = 0.0  # set erev according to the source cell (excitatory: 0, inhibitory: -80)
    else:
...
def main():
    ...
    parser.add_argument(
        "--pre", type=str, dest="pre", default="None", help="Select presynaptic cell for runone"
    )
    parser.add_argument(
        "--post", type=str, dest="post", default="None", help="Select postsynaptic cell for runone"
    )
    ...
    parser.add_argument(
        "-t", "--test", action="store_true", dest="test", help="Run the test on exp2syn fitting"
    )
    parser.add_argument(
        "-p",
        "--plot",
    ...
    parser.add_argument(
        "-l", "--list", action="store_true", dest="list", help="List the simple fit parameters"
    )
